...
-- ============================================
-- OpenComputers key_down is: address, character, keyCode, playerName.
-- Keep all key normalization here so GUI screens do not implement their own
-- event/polling/latch logic.
...
local function key(name, fallback)
    if keyboard.keys and keyboard.keys[name] ~= nil then
        return keyboard.keys[name]
    end
    return fallback
end
...
M.keys = {
    enter = key("enter", 28),
    numpadenter = key("numpadenter", 156),
    up = key("up", 200),
    down = key("down", 208),
    left = key("left", 203),
    right = key("right", 205),
...
function M.is(eventData, name)
    ...
    local code = eventData.code
    local char = M.char(eventData)
    if name == "enter" then return code == M.keys.enter or code == M.keys.numpadenter or code == 28 end
    if name == "back" then return code == M.keys.back or code == 14 or char == "b" or char == "B" end
